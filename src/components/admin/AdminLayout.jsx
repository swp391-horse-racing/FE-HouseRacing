...
export default function AdminLayout({
  ...
  return (
    <div className="min-h-screen bg-[#071426] font-sans text-white">
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex w-[270px] flex-col border-r border-white/10 bg-[#111f3b] transition-transform duration-200 ${open ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}
      >
        <Link
          to="/admin"
          ...
          {ADMIN_NAV.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex h-16 items-center gap-5 rounded-2xl border px-5 text-base font-semibold transition ${active ? "border-[#d4a017]/40 bg-[#d4a017]/15 text-white shadow-lg shadow-[#d4a017]/10" : "border-transparent text-white/60 hover:bg-white/5 hover:text-white"}`}
              >
