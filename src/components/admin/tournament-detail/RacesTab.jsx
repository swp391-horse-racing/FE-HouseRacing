...
export default function RacesTab({ tournament, setTournament }) {
  ...
  const addRace = () => {
    const race = createNewRace(tournament);
  ...
  return (
    <div className="grid gap-7 xl:grid-cols-[360px_1fr]">
      ...
      <div className="space-y-6">
        ...
        />
        {panel === "info" && (
          <RaceInfo race={selected} updateRace={updateRace} />
        )}
        {panel === "prizes" && (
          <RacePrizes race={selected} updateRace={updateRace} />
        )}
...
function RaceSidebar({ races, selected, onAdd, onSelect }) {
  return (
...
function RacePanelNavigation({ panel, race, onChange, onRemove }) {
  const panels = [
    ["info", "Thông tin", Info],
      ...
      <div className="mb-5 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-white/50">
              {race.date} · {race.time} · {race.distance}
            </p>
          </div>
        </div>
        <button
...
function RaceRegistrations({ race }) {
  ...
  return (
    <Card>
      ...
      />
      <SimpleTable
        headers={["Ngựa", "Chủ ngựa", "Jockey", "Tiền phí", "Duyệt"]}
