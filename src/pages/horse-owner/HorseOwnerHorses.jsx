...
import { GlassCard, Pill, PrimaryButton, GhostButton, TextInput } from '../admin/AdminLayout';
...
export function HorseOwnerHorses() {
  ...
  const [form, setForm] = useState({ name: '', breed: '', age: '', weight: '', color: '', health: 'Tốt' });
  ...
  const openAdd = () => {
    setEditTarget(null);
    setForm({ name: '', breed: '', age: '', weight: '', color: '', health: 'Tốt' });
    setShowModal(true);
  };

  const openEdit = (h) => {
    setEditTarget(h);
    setForm({ name: h.name, breed: h.breed, age: String(h.age), weight: String(h.weight), color: h.color, health: h.health });
    setShowModal(true);
  };

  const handleSave = () => {
    if (!form.name.trim()) { toast.error('Vui lòng nhập tên ngựa'); return; }
    if (editTarget) {
      setHorses((prev) =>
        prev.map((h) =>
          h.id === editTarget.id
            ? { ...h, name: form.name, breed: form.breed, age: Number(form.age), weight: Number(form.weight), color: form.color, health: form.health, healthTone: form.health === 'Tốt' ? 'green' : 'gold' }
            : h
        )
      );
  ...
  return (
    <HorseOwnerLayout
      ...
      actions={
        <PrimaryButton icon={Plus} onClick={openAdd}>Thêm ngựa</PrimaryButton>
      }
    >
            ...
            <div className="p-5">
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <p className="text-[12px] text-white/50 mt-0.5">{h.breed} · {h.color}</p>
                </div>
                <div className="flex gap-1.5">
                  <button onClick={() => openEdit(h)} className="p-1.5 bg-white/5 hover:bg-[#D4A017]/15 rounded-lg transition-all">
                    <Edit2 className="w-3.5 h-3.5 text-white/60 hover:text-[#D4A017]" />
                  </button>
                  <button onClick={() => handleDelete(h.id)} className="p-1.5 bg-white/5 hover:bg-red-500/15 rounded-lg transition-all">
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                <InfoItem label="Tuổi" value={`${h.age} tuổi`} />
                <InfoItem label="Cân nặng" value={`${h.weight} kg`} />
                <InfoItem label="Jockey" value={h.jockey ?? 'Chưa chọn'} />
                <InfoItem label="Trạng thái" value={h.status} />
              </div>

              <div className="flex items-center gap-4 p-3 bg-white/[0.04] rounded-xl">
                <div className="text-center flex-1">
                  <div className="text-lg font-bold text-[#D4A017]">{h.wins}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <GlassCard className="w-full max-w-md">
            <div className="p-5 border-b border-white/10 flex items-center justify-between">
              <h2 className="font-bold text-white">{editTarget ? 'Chỉnh sửa thông tin ngựa' : 'Thêm ngựa mới'}</h2>
              <button onClick={() => setShowModal(false)} className="p-1.5 hover:bg-white/10 rounded-lg transition-all">
            ...
            <div className="p-5 border-t border-white/10 flex items-center justify-end gap-3">
              ...
              <PrimaryButton onClick={handleSave}>{editTarget ? 'Lưu thay đổi' : 'Thêm ngựa'}</PrimaryButton>
            </div>
          </GlassCard>
        </div>
      )}
