...
export function HorseOwnerNotifications() {
  ...
  const filtered = filter === 'Chưa đọc' ? notifs.filter((n) => !n.read) : notifs;
  ...
  return (
    <HorseOwnerLayout
      ...
      actions={
        unread > 0 ? (
          <GhostButton icon={Check} onClick={markAll}>Đánh dấu tất cả đã đọc</GhostButton>
        ) : undefined
      }
    >
        ...
        {['Tất cả', 'Chưa đọc'].map((f) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-xl text-sm font-semibold transition-all ${
              filter === f ? 'bg-[#D4A017] text-white shadow-lg shadow-[#D4A017]/30' : 'bg-white/5 text-white/60 hover:bg-white/10 border border-white/10'
            }`}
          >
            {f} {f === 'Chưa đọc' && unread > 0 && <span className="ml-1 bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full">{unread}</span>}
          </button>
        ))}
      </div>
              ...
              <div className="p-4 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    {!n.read && <span className="w-2 h-2 bg-[#D4A017] rounded-full flex-shrink-0" />}
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  {!n.read && (
                    <button onClick={() => markRead(n.id)} className="p-1.5 hover:bg-white/10 rounded-lg transition-all" title="Đánh dấu đã đọc">
                      <Check className="w-3.5 h-3.5 text-white/50" />
                    </button>
                  )}
                  <button onClick={() => deleteNotif(n.id)} className="p-1.5 hover:bg-red-500/10 rounded-lg transition-all" title="Xóa">
