    ...
    >
        ...
        {[
          {
            ...
            icon: Users,
            tone: "bg-[#dda50e]/15 text-[#dda50e]",
          },
          {
            label: "Đang hoạt động",
            value: userRows.filter((u) => u.status === "Đang hoạt động").length,
            icon: UserCheck,
            tone: "bg-emerald-500/15 text-emerald-300",
          },
          {
            label: "Tạm khóa",
            value: userRows.filter((u) => u.status === "Tạm khóa").length,
            icon: UserX,
            tone: "bg-rose-500/15 text-rose-300",
          },
          {
            label: "Chờ duyệt quyền",
            value: requestRows.filter((r) => r.status === "Chờ duyệt").length,
            icon: BadgeCheck,
      ...
      <section className="mb-6 flex gap-3">
        <button
          ...
          className={`rounded-2xl px-5 py-3 text-sm font-semibold transition ${section === "users" ? "bg-[#dda50e] text-white shadow-lg shadow-[#d4a017]/30" : "border border-white/10 bg-white/[0.04] text-white/60 hover:text-white"}`}
        >
        ...
        <button
          ...
          className={`rounded-2xl px-5 py-3 text-sm font-semibold transition ${section === "requests" ? "bg-[#dda50e] text-white shadow-lg shadow-[#d4a017]/30" : "border border-white/10 bg-white/[0.04] text-white/60 hover:text-white"}`}
        >
          Yêu cầu cấp quyền
        </button>
      </section>
                ...
                <tbody>
                  {filteredUsers.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-5">
                        <span
                          className={`inline-flex rounded-full border px-3 py-1 text-sm font-semibold ${pillTone(item.role)}`}
                        >
                          {item.role}
                        </span>
                      </td>
                      <td className="px-6 py-5">
                        <span
                          className={`inline-flex rounded-full border px-3 py-1 text-sm font-semibold ${pillTone(item.status)}`}
                        >
                          {item.status}
                        </span>
                      </td>
                  ...
                  >
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-3">
                        <span
                          className={`inline-flex rounded-full border px-3 py-1 text-sm font-semibold ${pillTone(item.from)}`}
                        >
                          {item.from}
                        </span>
                        <Shield className="h-4 w-4 text-white/35" />
                        <span
                          className={`inline-flex rounded-full border px-3 py-1 text-sm font-semibold ${pillTone(item.to)}`}
                        >
                          {item.to}
                        </span>
                      </div>
